...
impl LsmStorageState {
    ...
    pub fn get_sstables(&self, ids: &[usize]) -> Result<Vec<Arc<SsTable>>> {
        ids.iter()
            .map(|id| Ok(Arc::clone(self.sstables.get(id).context("missing id")?)))
...
impl LsmStorageInner {
    ...
    fn replay_manifest_records(&self, records: Vec<ManifestRecord>) -> Result<()> {
        ...
        if self.options.enable_wal {
            ...
            for id in memtable_ids.iter().rev().skip(1).copied() {
                snapshot
                    .imm_memtables
            ...
            if let Some(id) = memtable_ids.last() {
                snapshot.memtable =
                    Arc::new(MemTable::recover_from_wal(*id, self.path_of_wal(*id))?);
        ...
        for record in batch {
            match record {
                WriteBatchRecord::Put(key, value) => {
                    read_guard.memtable.put(key.as_ref(), value.as_ref())
                }
                WriteBatchRecord::Del(key) => read_guard.memtable.put(key.as_ref(), b""),
            }?;
        }
        Ok(())
    }
    ...
    pub fn force_freeze_memtable(&self, state_lock: &MutexGuard<'_, ()>) -> Result<()> {
        let memtable = self.create_memtable()?;

        if self.options.enable_wal
            && let Some(ref manifest) = self.manifest
        {
            manifest.add_record(state_lock, ManifestRecord::NewMemtable(memtable.id()))?;
        }
    ...
    fn create_memtable(&self) -> Result<MemTable> {
    ...
    ) -> Result<TwoMergeIterator<MergeIterator<SsTableIterator>, MergeIterator<SstConcatIterator>>>
    {
        ...
        for id in state.l0_sstables.iter() {
            ...
            if !check_bloom_may_contain(table) {
                continue;
            }
